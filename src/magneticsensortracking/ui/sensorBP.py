...
class SensorRouting(socketio.AsyncNamespace):
    def __init__(
        self, sensor_group: sensors.base.SensorGroup, maxlen, *args, **kwargs
    ):
    ...
    async def send_predicted_vals(self):
        try:
            while True:
                ...
                predicted, _ = await asyncio.gather(loop.run_in_executor(pool, minimize, x0, (mags, pos, M0, shape)), asyncio.sleep(0.5))
                data = {'pos': {'x': predicted[0], 'y': predicted[1], 'z':predicted[2]},
...
def minimize(x0, args):
    ...
    res = sp.optimize.minimize(fun=cost_dipole, x0=x0, args=args, tol=1e-100, constraints=cons, bounds=bounds).x#, options={'maxiter': 1000}).x
